...
class KeyTeleop():
    ...
    def __init__(self, interface):
        ...
        self._pub_cmd = rospy.Publisher('/robot0/cmd_vel', Twist)
    ...
    def _get_twist(self, linear, angular):
        twist = Twist()
        if linear >= 0:
            twist.linear.x = self._forward(1, linear)
        else:
            twist.linear.x = self._backward(-1, -linear)
    ...
    def _key_pressed(self, keycode):
        movement_bindings = {
            curses.KEY_UP:    ( 0.2,  0),
            curses.KEY_DOWN:  (-0.2,  0),
            curses.KEY_LEFT:  ( 0,  0.2),
            curses.KEY_RIGHT: ( 0, -0.2),
        }
        speed_bindings = {
            ord(' '): (0, 0),
        }
